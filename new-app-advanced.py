...
st.title("🎙️ Local Whisper Transcriber with Cerebras")

st.write(
    "Record audio below and transcribe it locally using Whisper (faster-whisper).\n"
    "Then clean up the transcript using a Cerebras LLM.\n\n"
    "**Advanced mode** injects Snowflake-specific YAML semantic context so the LLM can "
    "snap noisy terms to canonical Snowflake terminology."
)

# ---------------------------
# Helpers for YAML / vocab
# ---------------------------
def extract_vocab_from_yaml(yaml_data):
    """
    Recursively walk the YAML structure and collect all string leaves
    as a flat list of vocabulary terms.
    """
    vocab = []
...
def load_yaml_context():
    ...
    Returns:
        None if missing/invalid, otherwise:
        {
            "raw": <original YAML as dict>,
            "vocab": <flat list of all string leaves>
        }
    """
    ...
    try:
        ...
        vocab = extract_vocab_from_yaml(data)
        return {"raw": data, "vocab": vocab}
...
def refine_transcript_with_cerebras(
    ...
    (like YAML context, vocab lists, etc.) to a Cerebras LLM
    ...
    if semantic_views:
        for key, value in semantic_views.items():
            ...
            # Special handling for the flat vocabulary list
            if key == "canonical_vocabulary" and isinstance(value, list):
                joined = "\n".join(f"- {term}" for term in value)
                blocks.append(f"{key} (flat list of canonical terms):\n{joined}")
    ...
    system_prompt = (
        "You are a transcription post-processor for Snowflake-related speech.\n"
        "You receive raw automatic speech recognition (ASR) text and optional semantic context.\n\n"
        "The semantic context defines canonical Snowflake and AI terminology. "
        "Treat those terms as the source of truth for naming.\n\n"
        "Your tasks:\n"
        "- Fix spelling, grammar, and capitalization.\n"
        "- Add punctuation and sentence boundaries.\n"
        "- Where the transcript appears to refer to terms in the semantic context "
        "  (even approximately or phonetically), replace them with the exact canonical spelling.\n"
        "- Prefer existing terms from the semantic context over inventing new ones.\n"
        "- Preserve technical names and Snowflake-specific terminology as given.\n"
        "- Do NOT introduce new product or feature names unless clearly implied.\n\n"
        "Return only the cleaned text. No explanations."
    )

    user_content = f"""Raw transcript (noisy ASR output):
\"\"\"{raw_text}\"\"\"

Semantic context (canonical Snowflake terminology and related concepts):
{views_str}

Instructions:
- Make a best-effort to map any approximate or misspelled domain terms in the transcript
  to the closest matching canonical terms from the semantic context.
- If you are unsure, keep the original wording rather than hallucinating.
"""
...
advanced_mode = st.checkbox(
    "Enable advanced mode (use Snowflake YAML semantic context)", value=False
)

if advanced_mode:
    st.info(
        "Advanced mode is **ON** — Cerebras will receive extra Snowflake-specific "
        "semantic context from `snowflake_specific_terms.yaml` and try to snap noisy "
        "terms to canonical vocabulary."
    )
else:
    st.info(
        "Advanced mode is **OFF** — Cerebras will only see the raw transcript, "
        "with no additional semantic context."
    )
...
if st.session_state.raw_transcript:
    ...
    # In advanced mode, optionally show loaded YAML for your own visibility
    if advanced_mode and yaml_context is not None:
        with st.expander("View loaded Snowflake YAML semantic context"):
            st.json(yaml_context["raw"])
        with st.expander("View flat canonical vocabulary list"):
            st.write("\n".join(f"- {t}" for t in yaml_context["vocab"]))

    # ---------------------------
    # Cerebras cleanup button
    # ---------------------------
    if st.button("✨ Clean & correct with Cerebras"):
        # Build semantic views dict only if advanced mode is on
        if advanced_mode and yaml_context is not None:
            semantic_views = {
                "snowflake_specific_terms": yaml_context["raw"],
                "canonical_vocabulary": yaml_context["vocab"],
            }
        else:
            semantic_views = None  # No extra context in basic mode
